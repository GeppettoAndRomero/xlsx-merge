...
`xlsx-merge` runs entirely in your browser. There is no server component and no
account system, so your workbooks are not uploaded. Most classic web vulnerabilities
...
- The workbook parsing and writing pipeline, the service worker, and the PWA manifest.
- Anything that could send workbook data or metadata off the device.
