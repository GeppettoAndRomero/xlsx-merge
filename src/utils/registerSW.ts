...
export async function registerServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  try {
    // Service Workerを登録（/xlsx-merge/sw.js, scope=/xlsx-merge/）。
    // scope がツールの全 locale ページ（/xlsx-merge/, /xlsx-merge/ja/ …）を被覆する。
    const registration = await navigator.serviceWorker.register('/xlsx-merge/sw.js', {
      scope: '/xlsx-merge/',
    });
